...
class ExtractionError(Exception):
    """Raised when LLM-based fact extraction fails.

    借鉴自 mem0 #5878 (fix(memory): re-raise LLM extraction failures instead
    of returning []): 之前 _extract_facts 在 LLM 调用失败时静默返回原始文本，
    导致上游无法区分「LLM 不可用(429/5xx/超时)」与「确实没抽出事实」。
    改为显式抛出 ExtractionError 并保留原始异常作为 cause，让调用方可以
    实现重试 / provider 回退 / 上报，而不是把整条用户消息当成一条事实吞掉。
    """
...
class AdditiveExtractionPipeline:
    ...
    def _extract_facts(self, text: str) -> List[str]:
        ...
        try:
            ...
            # 借鉴自 mem0 #5878: 不再静默返回原始文本。
            # 静默兜底会让上游无法区分「LLM 不可用」与「确实没抽出事实」，
            # 且会把整条用户消息污染成一条事实。改为显式抛出并保留原始异常。
            log.error("LLM fact extraction failed: %s", e)
            raise ExtractionError(f"LLM fact extraction failed: {e}") from e
