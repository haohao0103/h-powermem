...
# Word fragments aligned with unicode61-style token characters. Mirrors
# PowerMem's ``_sanitize_fts5_input`` (#1125): extract Unicode word tokens so
# full-text queries with punctuation-heavy or operator-like tokens don't
# silently void the full-text channel. Source: oceanbase/powermem@b1930b6.
_FULLTEXT_TOKEN_RE = re.compile(r"[\w]+", re.UNICODE)


def sanitize_fulltext_query(query: str) -> str:
    """Extract Unicode word tokens for the BM25 full-text path.

    Mirrors PowerMem's ``_sanitize_fts5_input`` (#1125): punctuation-heavy
    inputs (``v1.1.6``, ``C++``) and boolean operators (``AND``/``OR``/``NOT``)
    are reduced to literal word tokens instead of being treated as query
    syntax. Returns an empty string when nothing remains so the caller can
    skip the full-text channel gracefully (and log at debug) rather than
    crashing or silently dropping the full-text recall path.
    """
    query = str(query or "").strip()
    if not query:
        return ""
    tokens = _FULLTEXT_TOKEN_RE.findall(query)
    return " ".join(tokens)
...
class BM25FullTextBackend:
    ...
    def _tokenize(self, text: str, min_token_len: int = 2) -> List[str]:
        """Tokenize text using jieba, keeping CJK characters and alphanumerics.

        Args:
            text: Raw text to tokenize.
            min_token_len: Minimum token length to keep. Indexing uses the
                default (2) to keep the index compact; search uses 1 so
                short/symbol-heavy queries (e.g. ``"C++"`` → ``"c"``) still
                attempt a match instead of silently voiding the channel.
        """
        text = str(text)
        text = re.sub(r"[^\u4e00-\u9fa5a-zA-Z0-9]", " ", text)
        tokens = [
            t.strip().lower()
            for t in self._jieba.lcut(text)
            if len(t.strip()) >= min_token_len
        ]
        return tokens
    ...
    def search(self, query: str, top_k: int = 10, min_score: float = 0.0) -> List[Dict[str, object]]:
        ...
        # Sanitize first so punctuation-heavy / operator-like queries don't
        # silently void the full-text channel (mirrors powermem #1125).
        sanitized = sanitize_fulltext_query(query)
        if not sanitized:
            log.debug("BM25 full-text query yielded no tokens after sanitization: %r", query)
            return []

        query_tokens = self._tokenize(sanitized, min_token_len=1)
        if not query_tokens:
            return []
