...
# 借鉴自 PowerMem #1130 (fix(config): honor disabled graph store flag)
# 当 graph store 被显式禁用时，后端应优雅跳过图存储的初始化与读写，
# 仅用向量/BM25/SQLite 继续提供记忆能力，而不是因 HugeGraph 不可达而整体崩溃。
def _is_disabled_graph_store_flag(value) -> bool:
    """Return True when a graph-store flag value means 'disabled'.

    Mirrors PowerMem configs._is_disabled_graph_store_flag: accepts
    None/bool/str/int and treats False/0/"false"/"0"/"no"/"off"/"disabled" as off.
    """
    if value is None:
        return False
...
GRAPH_STORE_ENABLED = os.environ.get("GRAPH_STORE_ENABLED", "true")
...
class MemoryPipelineBackend:
    ...
    def __init__(self, hg_client: HugeGraphMemoryClient = None,
        ...
        # 借鉴自 PowerMem #1130: 仅在 graph store 启用时初始化 schema，
        # 否则跳过（HugeGraph 不可达也不应阻断向量/BM25/SQLite 记忆能力）。
        self.enable_graph = not _is_disabled_graph_store_flag(GRAPH_STORE_ENABLED)
        if self.enable_graph:
            try:
                self.hg.init_schema()
            except Exception as e:
                print(f"[HugeGraph] init_schema failed: {e}; disabling graph store at runtime",
                      file=sys.stderr, flush=True)
        ...
        # 仅当 enable_graph 为 True 时创建；禁用时置 None，add/search 跳过图通道。
        self._graph_store = (
            HugeGraphGraphStore(self.hg, max_hops=2, max_neighbors=50)
