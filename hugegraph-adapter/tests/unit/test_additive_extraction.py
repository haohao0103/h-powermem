...
class TestAdditiveExtractionPipeline:
    ...
    def test_llm_failure_raises_extraction_error(self):
        """LLM failure must propagate as ExtractionError (借鉴 mem0 #5878).

        静默回退到原始文本会掩盖 LLM 不可用(429/5xx/超时)，并把整条
        用户消息污染成一条事实。调用方必须能区分「LLM 挂了」与「确实没抽出事实」。
        """
    ...
    def test_no_llm_still_falls_back_to_raw_text(self):
        """No-LLM path keeps the deliberate fallback (treat text as one fact)."""
        pipeline = AdditiveExtractionPipeline(llm_callback=None)
