...
# ── P0: Graph-store disabled flag (PowerMem #1130) ───────────────────────────


@pytest.fixture
def memory_backend_graph_disabled(tmp_path, monkeypatch):
    """Backend built with GRAPH_STORE_ENABLED=false — must skip graph store."""
    import hugegraph_llm.poc.memory_backend as mb_module
    monkeypatch.setattr(mb_module, "GRAPH_STORE_ENABLED", "false")
    old_db_path = os.environ.get("MEMORY_DB_PATH")
    old_llm_key = os.environ.get("LLM_API_KEY")
    os.environ["MEMORY_DB_PATH"] = str(tmp_path / "meta.db")
    os.environ["LLM_API_KEY"] = "test-key"
    with tempfile.TemporaryDirectory() as tmpdir:
        os.environ["MEMORY_DATA_DIR"] = tmpdir
        memory_settings.memory_db_path = str(tmp_path / "meta.db")
        try:
            with mock.patch("hugegraph_llm.poc.memory_backend.FaissMemoryIndex"), \
                 mock.patch("hugegraph_llm.poc.memory_backend.HugeGraphMemoryClient") as MockHG:
                backend = MemoryPipelineBackend()
                backend._query_rewrite = mock.MagicMock()
                backend._query_rewrite.rewrite.return_value = {
                    "rewritten": "query", "entities": [], "intent": "general",
                    "method": "rule", "boosts": {},
                }
                backend._route_store = mock.MagicMock()
                backend._route_store.add_memory.return_value = "user:demo_user"
                ...
                backend._profile_injector = mock.MagicMock()
                backend._profile_injector.get_profile_for_rewrite.return_value = {
                    "user_profile": "", "aliases": {}, "topics": [],
                }
                backend._faiss_deletable = mock.MagicMock()
                backend._faiss_deletable.remove_by_id.return_value = True
                backend._compressor = mock.MagicMock()
                backend._compressor.compress.return_value = {
                    "summaries": [], "kept": [], "pruned": [], "archived": [], "stats": {},
                }
                backend._agent_manager = mock.MagicMock()
                backend._agent_manager.check_access.return_value = True
                backend._agent_manager.get_accessible_memories.return_value = []
                yield backend, MockHG
        finally:
            memory_settings.memory_db_path = None
            if old_db_path is not None:
                os.environ["MEMORY_DB_PATH"] = old_db_path
...
def test_backend_graph_store_disabled_skips_graph(memory_backend_graph_disabled):
    """PowerMem #1130: GRAPH_STORE_ENABLED=false → graph store skipped, core works."""
    backend, MockHG = memory_backend_graph_disabled
    # Graph store must be disabled and not created
    assert backend.enable_graph is False
    assert backend._graph_store is None
    # init_schema must NOT be called when graph disabled (HugeGraph unreachable safe)
    MockHG.return_value.init_schema.assert_not_called()

    # add_memory must still work (vector/BM25/SQLite path), no graph touched
    additive_mock = mock.MagicMock()
    additive_mock.run.return_value = {
        "new_facts": ["User likes tea"],
        "duplicate_facts": [],
        "entities": [{"name": "tea", "type": "concept"}],
        "hashes": {content_hash_md5("User likes tea")},
    }
    backend._additive_pipeline = additive_mock
    backend._llm_extract = mock.MagicMock(return_value={
        "entities": [{"name": "tea", "type": "concept"}], "relationships": [],
    })
    result = backend.add_memory("User likes tea", user_id="demo_user", skip_index_save=True)
    assert result["action"] == "ADD"
    # graph client must not be touched when disabled
    backend.hg.add_vertex.assert_not_called()
    backend.hg.add_edge.assert_not_called()

    # search_memory must still work and skip graph channel
    backend.faiss.search.return_value = [{
        "memory_id": result["memory_id"], "content": "User likes tea",
        "raw_score": 0.9, "weighted_score": 0.9,
    }]
    backend._bm25 = mock.MagicMock()
    backend._bm25.search.return_value = []
    sres = backend.search_memory("tea", user_id="demo_user", fast_eval=True)
    assert "results" in sres
